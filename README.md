...
A microservice sample for building an e-commerce backend. Medium article write-up on this project can be found here [here](building-scalable-e-commerce-backend-with-microservices-exploring-design-decisions-node-js-b5228080403b)
...
- Interactions between `product` service and `order` service uses  protcol, using RabbitMQ which consists of two queues - `orders` and `products`. This saves on resources allocated for REST calls to MongoDB.
